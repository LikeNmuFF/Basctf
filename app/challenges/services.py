...
def rate_challenge(user_id: int, challenge_id: int, rating_value: bool) -> tuple[bool, str]:
    ...
    try:
        ...
        if not challenge or not challenge.is_active:
            return False, {'message': 'Challenge not found or not available'}
        ...
        # Check if user already rated this challenge with fresh query
        ...
        if existing_rating:
            ...
            # Create new rating - use try/except for race condition
            try:
                ...
                # Roll back and fetch the existing rating
                db.session.rollback()
                existing_rating = ChallengeRating.query.filter_by(
                    user_id=user_id,
                    challenge_id=challenge_id
                ...
                if existing_rating:
                    # Now update the one that was created by the other request
                    if existing_rating.rating == rating_value:
                    ...
                    return False, {'message': 'Rating conflict - please try again'}
        ...
        print(f"Error rating challenge: {e}")
        return False, {'message': f'Error rating challenge: {str(e)}'}
...
def remove_challenge_rating(user_id: int, challenge_id: int) -> tuple[bool, str]:
    ...
    try:
        ...
        if not rating:
            return False, {'message': 'You have not rated this challenge'}
        ...
        return False, {'message': f'Error removing rating: {str(e)}'}
